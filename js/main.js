// citylist.js
// A JS tool that helps developers to deal with list of all Indian states and cities dropdown list
// Made with JS only (no library required)

// Author - Nitin Nair
// GitHub - https://github.com/nitin9nair 

// Below stateList json object forked from github user repo "indian-list" - https://github.com/bhanuc
// Indian state json object start
...
// Indian state json object end

// disabling state and city select boxes (enabled after country select box value is selected)
document.getElementById("stateSelector").disabled = true;
document.getElementById("citySelector").disabled = true;

let countryList = document.getElementById("countrySelector");
...
function selectCountry() {
    ...
    if (selectedCountry == "India") {
        ...
        // calling function to generate states based on country (default - India)
        generateState();
    }
}


// function to generate list of all states of a particular country (India)
function generateState() {
    let stateList = document.getElementById("stateSelector");

    // dynamically creating options for the state select box
    for (let i = 0; i < Object.keys(states).length; i++) {
        let opt = document.createElement("option");
        opt.value = Object.keys(states)[i];
        opt.textContent = Object.keys(states)[i];
...
// function to get selected state value
function selectState() {
    ...
    if (cityNodeCheck.childElementCount > 1) {
        ...
        document.getElementById("displayCity").textContent = null;
    }

    // validate correct value
    if (selectedState !== "default") {
        document.getElementById("citySelector").disabled = false;

        // calling function to generate states based on country (default - India)
        generateCity(selectedState);
    }
}
...
// function to get selected city value
function selectCity() {
